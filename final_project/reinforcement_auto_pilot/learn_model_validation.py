    ...
    while t < train_frames:
        ...
        if t > observe:
            ...
            model.fit(x_train, y_train, batch_size=batchSize,
                      epochs=1, verbose=0, callbacks=[history], validation_data=(x_test, y_test))
        ...
        if t % 25000 == 0:
            model.save_weights('saved-models-valid/' + filename + '-' +
                               str(t) + '.h5',
                               overwrite=True)
...
def log_results(filename, data_collect, loss_log):
    ...
    with open('results/frames-valid/learn_data-' + filename + '.csv', 'w') as data_dump:
        wr = csv.writer(data_dump)
        wr.writerows(data_collect)

    with open('results/frames-valid/loss_data-' + filename + '.csv', 'w') as lf:
        wr = csv.writer(lf)
...
def launch_learn(params):
    ...
    if not os.path.isfile('results/frames-valid/loss_data-' + filename + '.csv'):
        # Create file so we don't double test when we run multiple
        # instances of the script at the same time.
        open('results/frames-valid/loss_data-' + filename + '.csv', 'a').close()
        print("Starting test.")
        # Train.
