...
class InputPreprocessor:
    """
        Constructor for InputPreprocessor class
    """
    ...
    def __call__(self, obs):
        ...
        all_meas_list = [
            np.array(obs[m]) if (isinstance(obs[m], list) or isinstance(obs[m], np.ndarray)) else np.array([obs[m]]) for
            m in self.meas_list]
        ...
        return {'image': self.img_buffer[np.arange(self.step - 1,
                                                   self.step - 1 + self.n_images_to_accum) % self.n_images_to_accum].astype(
            np.float32) / 255. - 0.5, \
