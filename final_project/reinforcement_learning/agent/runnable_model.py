...
import sys
import os
from .asynchronous_model import run_train_test
from .asynchronous_model import a3c
import numpy as np
...
class A3CAgent(Agent):
    ...
    def compute_n_meas(self, args):
        modalities = ['accel_x', 'accel_y', 'collision_car', 'collision_gen', 'collision_ped', 'game_timestamp',
                      'platform_timestamp', 'image', 'ori_x', 'ori_y', 'ori_z', 'player_x', 'player_y',
                      'intersect_otherlane', 'intersect_offroad', 'speed', 'vector_to_goal', 'distance_to_goal',
                      'planner_command', 'step']
    ...
    def setup_model(self, n_actions, n_meas, args):
        ...
        if type(self.model).__name__ == 'A3CFF':
            ...
            raise Exception('Unknown model type', type(self.model).__name__)

        if (not hasattr(args, 'carla_action_set')) or args.carla_action_set == '9':
            self.actions = [[0., 0.], [-1.,0.], [-0.5,0.], [0.5, 0.], [1.0, 0.],
                            [0., -1.], [0., -0.5], [0., 0.5], [0.,1.]]
        elif args.carla_action_set == '13':
            self.actions = [[0., 0.], [-1.,0.], [-0.5,0.], [-0.25,0.], [0.25,0.], [0.5, 0.], [1.0, 0.], [0., -1.],
    ...
    def setup_data_preprocessor(self, args):
        meas_coeffs_dict = {'step': [1/500.], 'vector_to_goal': [1/5000.,1/5000.], 'distance_to_goal': [1/1000.],
                            'speed': [1/10.], 'collision_gen': [1/500000.], 'collision_ped': [1/100000.],
                            'collision_car': [1/500000.], 'intersect_offroad': [1.], 'intersect_otherlane': [1.],
                            'planner_command': [0.5, 0.5, 0.5, 0.5, 0.5]}
        meas_coeffs = np.concatenate([np.array(meas_coeffs_dict[m]) for m in args.meas_list]).astype(np.float32)
        self.input_preprocessor = run_train_test.InputPreprocessor(meas_coeffs=meas_coeffs,
                                                                   n_images_to_accum=args.n_images_to_accum,
                                                                   meas_list=args.meas_list)
    ...
    def data_from_simulator_to_dict(self, measurements, sensory, planner_command, target):
        modalities = ['accel_x', 'accel_y', 'collision_car', 'collision_gen', 'collision_ped', 'game_timestamp',
                      'platform_timestamp', 'image', 'ori_x', 'ori_y', 'ori_z', 'player_x', 'player_y',
                      'intersect_otherlane', 'intersect_offroad', 'speed', 'vector_to_goal',
                      'distance_to_goal', 'planner_command']
        if measurements is None:
            ...
            if data_dict['planner_command'] is None:
                ...
                assert (data_dict['planner_command'] in [0., 2., 3., 4., 5.]), \
                    'Got planner command {}. Expected to be one of: 0,2,3,4,5'.format(data_dict['planner_command'])
